...
def run_embed(keywords):
    ...
    if Embed.keywords['occupied_shells'] != 0:
        Cspan_0, Ckern_0 = Orbs.initial_shell(S, Cenv, Embed.n_aos)
        if Embed.keywords['occupied_shells'] > 1:
            Cspan_old = Cspan_0
            Ckern_old = Ckern_0
            for i in range(Embed.keywords['occupied_shells'] - 1):
                Cspan_i, Ckern_i, A_i =  Orbs.build_shell(F_emb, Cspan_old, Ckern_old, shell)
                Cspan_i =  np.hstack((Cspan_i,Cspan_old))
                Cspan_old = Cspan_i
                Ckern_old = Ckern_i
            Cact = np.hstack((Cact, Cspan_i))
        ...
        if Embed.keywords['n_shells'] == 0:
            ...
            if n_shells > 1:
                for i in range(n_shells - 1):
                    ...
                    np.savez('shell{:03}.npz'.format(i), Cspan_i)
